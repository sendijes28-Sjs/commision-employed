...
export function Sidebar({ isMobileOpen, onCloseMobile }: { isMobileOpen?: boolean; onCloseMobile?: () => void }) {
  ...
  // PERF-1: Fetch lightweight badge counts instead of all invoices
  useEffect(() => {
  ...
  return (
    <>
      ...
      <aside
        ...
        } w-[260px] fixed md:sticky top-0 left-0 z-50 bg-slate-900 flex flex-col h-screen transition-all duration-300 ease-in-out border-r border-white/[0.06]`}
      >
        {/* Logo */}
                ...
                {section.items.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center ${collapsed && !isMobileOpen ? "justify-center" : "justify-between"} ${collapsed && !isMobileOpen ? "px-0 py-2.5" : "px-3 py-2"} rounded-lg transition-all duration-200 group ${
                        isActive
